...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/shuuiran/3x-ui/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_menu() {
    ...
    wget -O /usr/bin/x-ui https://raw.githubusercontent.com/shuuiran/3x-ui/main/x-ui.sh
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls "https://raw.githubusercontent.com/shuuiran/3x-ui/v$tag_version/install.sh") v$tag_version"
...
uninstall() {
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/shuuiran/3x-ui/master/install.sh)${plain}"
    echo ""
...
update_shell() {
    wget -O /usr/bin/x-ui -N https://github.com/shuuiran/3x-ui/raw/main/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
ssl_cert_issue_CF() {
    ...
    if [ $? -eq 0 ]; then
        ...
        ~/.acme.sh/acme.sh --issue --dns dns_cf -d ${CF_Domain} -d *.${CF_Domain} --dnssleep 60 --log --force
        if [ $? -ne 0 ]; then
